...
This guide shows the main workflows. For the full list of commands and flags, see the generated link:kubectl-plugin/cli-docs.adoc[command reference].
...
Pre-built binaries for `linux/amd64`, `linux/arm64`, `darwin/amd64`, and `darwin/arm64` are published on each https://github.com/kubewarden/runtime-enforcer/releases[GitHub Release].
...
Promoting a `WorkloadPolicyProposal` sets the `runtimeenforcer.kubewarden.io/promote=true` label and lets the controller create a `WorkloadPolicy`.
